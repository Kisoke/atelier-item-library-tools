...
module Processors
  module LibraryImagesTesseract
    ...
    def self.write_pattern(pattern, pattern_id)
      path =
        Pathname.new("/tmp/a23_extractor-#{PATTERNS_PARENT}-#{pattern_id}.txt")
    ...
    class ImageCategoryExtractor < ImageTextExtractor
      ...
      def initialize(image_path)
        @categories_pattern_path =
          LibraryImagesTesseract.write_pattern(CATEGORIES_PATTERN, "categories")
    ...
    class ImageEffectLevelExtractor < ImageTextExtractor
      ...
      def initialize(image_path)
        @levels_pattern_path =
          LibraryImagesTesseract.write_pattern(LEVELS_PATTERN, "levels")
